...
def create_spectrum(audio, sr, n_fft=8192, hop_length=2048):
    if np.isnan(audio).any() or np.isinf(audio).any() or np.isneginf(audio).any():
        ...
        audio = zeros
    spectrum = librosa.cqt(audio, sr = sr, fmin=constants.HP_FREQ, n_bins=240, hop_length=512, bins_per_octave=24)
    abs = np.abs(spectrum)
    power_mean = np.mean(abs, axis=1)
    power_dB = librosa.amplitude_to_db(power_mean)
